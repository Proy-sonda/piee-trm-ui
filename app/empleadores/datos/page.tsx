...
import { CCACTLABCB, CCAFCB, CCCOMUNACB, CCREGIONCB, CCREMUNERACION, CCTAMANOCB, CCTIPOEM } from '@/app/contexts/interfaces/types';
import useCombo from '@/app/hooks/useCombo';
import { EmpleadorContext } from '@/app/contexts/EmpleadorContext';
import { ClipLoader } from 'react-spinners';
import { ActualizaEmpleador } from '@/app/interface/tramitacion';
import { actualizaEmpleador } from '@/app/helpers/tramitacion/empleadores';
...
const DatosEmpleadoresPage = ({ searchParams }: DatosEmpleadoresProps) => {
  ...
  let { razon, templador, ccaf, alaboralemp, calle,
    numero, bdep, ccomuna, tf1, tf2, cemple, recemple,
    qtrabajadores, sremuneraciones, fantasia,
    onInputChange, onInputChangeOnlyNum } = useForm({
      ...
      sremuneraciones: empleador.find((v) => v.rutempleador == rut)?.sistemaremuneracion.idsistemaremuneracion,
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (empleador.find((v) => v.rutempleador == rut)?.rutempleador) {
        setregion(empleador.find((v) => v.rutempleador == rut)?.direccionempleador.comuna.idcomuna.substring(0, 2));
        CCCOMUNA = CCCOMUNA.filter(({ region: { idregion } }) => idregion == Number(empleador.find((v) => v.rutempleador == rut)?.direccionempleador.comuna.idcomuna.substring(0, 2)));
        setcomuna(CCCOMUNA);
      }
      setLoading(false);
    }, 1800);
  ...
  const handleSubmit = (e:FormEvent)=> {
      ...
      const ActEmpleador = async ()=> {
        ...
        if(resp.ok) return Swal.fire({html:'Entidad empleadora fue actualizado con exito', timer:2000, showConfirmButton:false, icon:'success'});
      ...
      <div className="container">
        ...
        <div className="row mt-2">
          <div className="col-md-8">
            <Stage manual="" url="#">
              Datos Entidad Empleadora - {  empleador.find((v)=> v.rutempleador == rut )?.razonsocial}
            </Stage>
          </div>
        ...
        <div className="row mt-2">
          <div className="col-md-4">
            ...
            <select className="form-select" name="templador" onChange={onInputChange} value={templador} required>
              <option value={''}>
                  Seleccionar
              </option>
              {
          ...
          <div className="col-md-4">
            ...
            <select className="form-select" name="ccaf" value={ccaf} onChange={onInputChange} required >
              <option value={''}>
                Seleccionar
              </option>
              ...
              {
                (CCAF.length > 0)
                ? CCAF.map((ccaf)=> <option key={ccaf.idccaf} value={ccaf.idccaf}> {ccaf.nombre}</option>)
                : <></>
          ...
          <div className="col-md-4">
            ...
            <select className="form-select" id="alaboralemp" name='alaboralemp' value={alaboralemp} onChange={onInputChange} required>
              <option value={''}>Seleccionar</option>
              {
                (CCACTLAB.length > 0)
                ? CCACTLAB.map(({idactividadlaboral, actividadlaboral})=> <option key={idactividadlaboral} value={idactividadlaboral}>{actividadlaboral}</option> )
                : <></>
              }
            </select>
          </div>
        ...
        <div className="row mt-2">
          <div className="col-md-4">
            ...
            <select className="form-select" id="region" value={region} onChange={onChangeRegion} required >
              <option value={''}>Seleccionar</option>
              {
                (CCREGION.length > 0)
          ...
          <div className="col-md-4">
            ...
            <select className="form-select" name='ccomuna' value={ccomuna} onChange={onInputChange} > 
            <option value={''}>Seleccionar</option>
            {
              comunas.length > 0
        ...
        <div className="row mt-2">
          ...
          <div className="col-md-4">
            <div className="form-group">
              ...
              <select className="form-select" id="qtrabajadores" value={qtrabajadores} onChange={onInputChange} required>
                <option value={''}>Seleccionar</option>
                {
                  CCTAMANOCB.length > 0
                  ? CCTAMANOCB.map(({idtamanoempresa, descripcion})=><option key={idtamanoempresa} value={idtamanoempresa}>{descripcion}</option>)
                  : <></>
                }
              </select>
            </div>
          ...
          <div className="col-md-4">
            ...
            <select className="form-select" id="sremuneraciones" value={sremuneraciones} onChange={onInputChange} required >
              <option value={''}>Seleccionar</option>
              {
